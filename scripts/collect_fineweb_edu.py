#!/usr/bin/env python3
"""Stream FineWeb-Edu raw English text until the exact token quota is full.

The pinned sample-100BT source is read one Parquet shard at a time through
Hugging Face streaming. Source Parquet is never copied to the dataset volume.
Accepted text is stored byte-for-byte in lossless tar.zst archives with an
internal provenance manifest. Checkpoints contain an exact source-file and row
cursor and are safe to resume after interruption.
"""
...
class RawTextArchiveWriter:
    def __init__(self, root: Path, index: int, compression_level: int, compression_threads: int):
        ...
        output_dir = root / "raw" / "english" / "fineweb_edu"
    ...
    def add(self, content: bytes, token_count: int, row: dict[str, Any]) -> None:
        ...
        manifest_row = {
            ...
            "fineweb_token_count": row.get("token_count"),
            "fineweb_edu_score": row.get("score"),
            "fineweb_edu_int_score": row.get("int_score"),
...
def next_archive_index(root: Path) -> int:
    directory = root / "raw" / "english" / "fineweb_edu"
    indexes = []
    if directory.exists():
...
def resolve_dataset_source(
    ...
    try:
        ...
        raise RuntimeError("Install huggingface_hub before collecting FineWeb-Edu") from exc
    if dataset_config not in CONFIG_PREFIXES:
        raise ValueError(f"Unsupported FineWeb-Edu config: {dataset_config}")
    manifest_path = root / "manifests" / "FINEWEB_EDU_SOURCE.json"
    ...
    if existing and (existing.get("repo_id") != repo_id or existing.get("dataset_config") != dataset_config):
        raise ValueError("Existing FineWeb-Edu source manifest uses a different repository/config")
    ...
    if card.get("language") != ["en"]:
        raise ValueError(f"FineWeb-Edu card is not English-only: {card.get('language')!r}")
    ...
    if existing:
        for key, value in fixed.items():
            if existing.get(key) != value:
                raise ValueError(f"Existing FineWeb-Edu manifest has a different {key}")
    atomic_json(
        manifest_path,
        {
            "manifest_version": 1,
...
def checkpoint_directory(root: Path) -> Path:
    return root / "state" / "fineweb_edu_checkpoints"
...
def recover_checkpoints(
    ...
    for path in sorted(directory.glob("checkpoint-*.json")):
        ...
        if checkpoint.get("checkpoint_version") != CHECKPOINT_VERSION:
            raise RuntimeError(f"Unsupported FineWeb-Edu checkpoint version in {path}")
        for key, expected in (
            ("source", source),
            ("tokenizer_revision", tokenizer_sha256),
            ("benchmark_guard_sha256", benchmark_guard_sha256),
            ...
            if checkpoint.get(key) != expected:
                raise RuntimeError(f"FineWeb-Edu checkpoint {key} mismatch in {path}")
        ...
        if archive:
            ...
            shard_id = f"fineweb-edu-{checkpoint['dataset_revision'][:12]}-{archive['index']:06d}"
            write_record(
                root,
                {
                    "phase": "collection",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--cache-dir", type=Path, default=Path("/tmp/fineweb-edu-cache"))
...
def main() -> int:
    ...
    try:
        ...
        if not 0.0 <= args.min_language_score <= 1.0:
            raise ValueError("--min-language-score must be between 0 and 1")
        ...
        try:
            ...
            raise RuntimeError("Install tokenizers before collecting FineWeb-Edu") from exc
        ...
        try:
            ...
            raise RuntimeError("Install datasets before collecting FineWeb-Edu") from exc
        ...
        accepted_bytes = 0
        skipped = {"empty": 0, "not_english": 0, "url_quarantine": 0, "benchmark_content": 0}
        rejections_path = args.root / "logs" / "fineweb_edu_benchmark_rejections.jsonl"
        ...
        for current_shard_index in range(source_shard_index, len(source_shards)):
            ...
            for row in stream:
                ...
                if not isinstance(text, str) or not text.strip():
                    skipped["empty"] += 1
                elif row.get("language") != "en" or float(row.get("language_score") or 0.0) < args.min_language_score:
                ...
                if reached or low_disk or pilot_done or stop_request.requested:
                    if reached:
                        atomic_json(
                            args.root / "state" / "ENGLISH_FINEWEB_EDU_COMPLETE.json",
                            {
                                "source": source,
        ...
        print("The configured FineWeb-Edu source ended before the English quota was reached.")
        return 1
