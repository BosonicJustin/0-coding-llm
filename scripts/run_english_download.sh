...
LOG_FILE="$LOG_DIR/fineweb_edu_collector.log"
...
echo "[$(date --iso-8601=seconds)] Starting/resuming FineWeb-Edu collection" | tee -a "$LOG_FILE"

set +e
"$PYTHON_BIN" "$PROJECT_ROOT/scripts/collect_fineweb_edu.py" \
  --root "$DATA_ROOT" \
...
set -e
echo "[$(date --iso-8601=seconds)] FineWeb-Edu collector exited with status $exit_code" | tee -a "$LOG_FILE"
exit "$exit_code"
